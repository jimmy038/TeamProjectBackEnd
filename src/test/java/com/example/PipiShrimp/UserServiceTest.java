...
public class UserServiceTest {
	...
	public void signUpTest() {
		...
		System.out.println("執行結果1:" + res.getRtnCode().getMessage());

		// 資料為空
		user = new User("", "ian@gmail.com", "");
		res = service.signUp(user);
		System.out.println("執行結果2:" + res.getRtnCode().getMessage());

		// 信箱已存在
		user = new User("jack", "ian0217@gmail.com", "ian1234fff");
		res = service.signUp(user);
		System.out.println("執行結果3:" + res.getRtnCode().getMessage());

		// 密碼格式不符
		user = new User("jack", "jack@gmail.com", "12346789");
		res = service.signUp(user);
		System.out.println("執行結果4:" + res.getRtnCode().getMessage());
